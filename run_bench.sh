...
do
	...
	srun --nodes=1 --ntasks=$mpi_node --cpus-per-task=$omp_thread ./$exe $args > /dev/null
	mv result.dat $path/$counter_${cc}_$mpi_node_$omp_thread.dat
	mv config.dat $path/$counter_${cc}_$mpi_node_$omp_thread.cfg
	counter+=1
