...
do
	...
	mv config.dat $path/gcc_config_$m.dat

	srun --cpus-per-task=$cpt ./prr_icc $args > /dev/null
	mv result.dat $path/icc_result_$m.dat
	mv config.dat $path/icc_config_$m.dat

	srun --ntasks=$ntasks --cpus-per-task=$cpt ./multiprr_gcc $args
	mv result.dat $path/multiprr_gcc_result_$m.dat
	mv config.dat $path/multiprr_gcc_config_$m.dat

	srun --ntasks=$ntasks --cpus-per-task=$cpt ./multiprr_icc $args
	mv result.dat $path/multiprr_icc_result_$m.dat
	mv config.dat $path/multiprr_icc_config_$m.dat
done
